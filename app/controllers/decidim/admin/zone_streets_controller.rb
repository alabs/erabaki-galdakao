    ...
    class ZoneStreetsController < GaldakaoController
      ...
      def create
        ...
        CreateGaldakaoZoneStreet.call(@form, zone) do
          on(:ok) do
            flash[:notice] = "Calle añadida correctamente"
            redirect_to decidim_admin.galdakao_zone_path(zone)
          end
          on(:invalid) do |error|
            flash.now[:alert] = "Error al añadir la calle: #{error}"
            render :new
          end
        end
      end
      ...
      def update
        ...
        UpdateGaldakaoZoneStreet.call(@form, zone_street) do
          on(:ok) do
            flash[:notice] = "Calle actualizada correctamente"
            redirect_to decidim_admin.galdakao_zone_path(zone)
          end
          on(:invalid) do |error|
            flash.now[:alert] = "Error al actualizar la calle: #{error}"
            render :edit
          end
        end
      end
      ...
      def destroy
        zone_street.destroy!
        flash[:notice] = "Calle eliminada correctamente"
        redirect_to decidim_admin.galdakao_zone_path(zone)
      end
